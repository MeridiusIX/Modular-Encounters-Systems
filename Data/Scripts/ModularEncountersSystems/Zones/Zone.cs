...
namespace ModularEncountersSystems.Zones {
	...
	public class Zone {
		...
		/// <summary>
		/// Copies definition fields from a zone profile onto this instance while preserving runtime state
		/// (Active, timers, encounter counts, custom values, players in zone, planet association).
		/// </summary>
		public void ApplyDefinitionFromProfile(Zone profile, PlanetEntity planet = null) {

			if (profile == null)
				return;

			// Preserve runtime state
			bool savedActive = Active;
			DateTime savedTimeCreated = TimeCreated;
			int savedSpawnedEncounters = SpawnedEncounters;
			var savedCounters = CustomCounters;
			var savedBools = CustomBools;
			var savedPlayers = PlayersInZone;
			long savedPlanetId = PlanetId;

			// Definition flags / identity
			Persistent = profile.Persistent;
			Strict = profile.Strict;
			PlayerKnownLocation = profile.PlayerKnownLocation;
			NoSpawnZone = profile.NoSpawnZone;
			Name = profile.Name;
			PublicName = profile.PublicName;
			ProfileSubtypeId = profile.ProfileSubtypeId;

			// Factions
			UseAllowedFactions = profile.UseAllowedFactions;
			AllowedFactions = profile.AllowedFactions != null
				? new List<string>(profile.AllowedFactions)
				: new List<string>();
			UseRestrictedFactions = profile.UseRestrictedFactions;
			RestrictedFactions = profile.RestrictedFactions != null
				? new List<string>(profile.RestrictedFactions)
				: new List<string>();

			// Geometry / placement definition
			Radius = profile.Radius;
			RadiusSquared = Radius * Radius;
			CoordinateRadiusPairs = profile.CoordinateRadiusPairs != null
				? new List<KeyValuePair<Vector3D, double>>(profile.CoordinateRadiusPairs)
				: new List<KeyValuePair<Vector3D, double>>();

			PlanetaryZone = profile.PlanetaryZone;
			PlanetName = profile.PlanetName;
			Direction = profile.Direction;
			HeightOffset = profile.HeightOffset;
			ScaleZoneRadiusWithPlanet = profile.ScaleZoneRadiusWithPlanet;
			IntendedPlanetSize = profile.IntendedPlanetSize;

			if (PlanetaryZone) {

				PlanetId = savedPlanetId;
				...
				// else keep existing Coordinates (planet missing at refresh time)
			...
			// Timer / encounter definition (not current progress)
			UseZoneTimer = profile.UseZoneTimer;
			MinutesToExpiration = profile.MinutesToExpiration;
			PlayerPresenceResetsTimer = profile.PlayerPresenceResetsTimer;
			UseMaxSpawnedEncounters = profile.UseMaxSpawnedEncounters;
			MaxSpawnedEncounters = profile.MaxSpawnedEncounters;

			// Spawn group / mod filters
			UseAllowedSpawnGroups = profile.UseAllowedSpawnGroups;
			AllowedSpawnGroups = profile.AllowedSpawnGroups != null
				? new List<string>(profile.AllowedSpawnGroups)
				: new List<string>();
			UseRestrictedSpawnGroups = profile.UseRestrictedSpawnGroups;
			RestrictedSpawnGroups = profile.RestrictedSpawnGroups != null
				? new List<string>(profile.RestrictedSpawnGroups)
				: new List<string>();

			UseAllowedModIDs = profile.UseAllowedModIDs;
			AllowedModIDs = profile.AllowedModIDs != null
				? new List<ulong>(profile.AllowedModIDs)
				: new List<ulong>();
			UseRestrictedModIDs = profile.UseRestrictedModIDs;
			RestrictedModIDs = profile.RestrictedModIDs != null
				? new List<ulong>(profile.RestrictedModIDs)
				: new List<ulong>();

			// Announcements / sandbox requirements
			UseZoneAnnounce = profile.UseZoneAnnounce;
			ZoneEnterAnnounce = profile.ZoneEnterAnnounce;
			ZoneLeaveAnnounce = profile.ZoneLeaveAnnounce;
			FlashZoneRadius = profile.FlashZoneRadius;
			RequiredSandboxBool = profile.RequiredSandboxBool;
			RequiredFalseSandboxBool = profile.RequiredFalseSandboxBool;

			// Restore runtime state
			Active = savedActive;
			TimeCreated = savedTimeCreated;
			SpawnedEncounters = savedSpawnedEncounters;
			CustomCounters = savedCounters ?? new Dictionary<string, long>();
			CustomBools = savedBools ?? new Dictionary<string, bool>();
			PlayersInZone = savedPlayers ?? new List<long>();
